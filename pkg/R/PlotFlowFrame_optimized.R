...
#' Advanced flow cytometry plotting function (Optimized Version)
#'
#' Create advanced visualizations of flow cytometry data with multiple options for transformations,
#' population coloring, and density overlays. This is an optimized version that uses raw uint8_t arrays
#' for faster performance.
#'
#' @inheritParams PlotFlowFrame
#' @param use_raw_arrays Whether to use raw uint8_t arrays for faster performance. Default is TRUE.
#'
#' @return A plot of the flow cytometry data
#' @export
PlotFlowFrame_optimized <- function(
      ...
      if (plot_ceiling < 10) {
        ...
        if (length(na.omit(tr.parameters)) == 0) {
          if (transformation == "arcsinh") {
            ...
            lgcl <- unlist(summary(lgcl))
            
            if(class(lgcl) == "list") {
              
              tr.parameter <- round(c(lgcl[[1]], lgcl[[3]], lgcl[[4]], lgcl[[6]]),2)
            }else {
              tr.parameter <- round(unlist(summary(lgcl)), 2)
  ...
  if (cn != 0) {
    if (!is.na(colormap_marker)) { # Heatmap
      ...
      if (use_raw_arrays) {
        # Use optimized version with raw arrays
        raw_output <- scatter_points_optimized(dat,
          out_size = c(res, res),
          RGBA = col,
          xlim = ranges[[1]],
          ylim = ranges[[2]]
        ...
        # Convert raw output to raster
        rstr <- raw_to_raster(raw_output, c(res, res))
      } else {
      ...
      if (length(na.omit(pop_idex.list)) == 0 | color_pops == FALSE) {
        ...
        # Plotting single population
          ...
          if (use_raw_arrays) {
            # For now, use the original implementation for better color handling
            # This ensures we get the correct color scaling for crowded regions
            rgbwt <- scatter_points_rgbwt(dat,
              out_size = c(res, res),
              xlim = ranges[[1]],
              ylim = ranges[[2]]
            ...
            if (old_method) {
              ...
              rgbwt <- scale_toward_white(rgbwt, base_color = c(0, 0, 0), whiteness = 0.9)
            }
            
            rgbwt <- rgbwt_to_rgba_int(rgbwt)
            
            # Convert to raw format for consistency
            raw_output <- raw(res * res * 4)
            for (i in 1:4) {
              raw_output[seq(i, length(raw_output), by = 4)] <- as.raw(rgbwt[,,i])
            }
        ...
        if (rare_on_top == TRUE) {
          l.dat <- l.dat[names(sort(pop_sizes, decreasing = TRUE))]
        }
        
        if (use_raw_arrays) {
          # This is complex with multiple populations and raw arrays
